// App.jsx (ANDROID) from the VAULT OPUS PROJECT version 1-R10
// ==================== FULL ANDROID GUI App.jsx ====================
// IF YOU WANT A DESKTOP VERSION GO TO https://github.com/WeDu-official/VAULT-OPUS
...
function MiniMarkdown({ content }) {
    ...
    return (
        <div className="space-y-4 text-sm text-gray-300 leading-relaxed font-sans text-left">
            {lines.map((line, idx) => {
                if (line.startsWith('# ')) return <h1 key={idx} className="text-xl font-black text-white mt-6 mb-3 border-b border-[#1a3a5c] pb-2 uppercase tracking-wide">{line.slice(2)}</h1>;
                if (line.startsWith('## ')) return <h2 key={idx} className="text-base font-bold text-[#3bb5ff] mt-5 mb-2 uppercase tracking-wider">{line.slice(3)}</h2>;
                if (line.startsWith('### ')) return <h3 key={idx} className="text-sm font-bold text-white mt-4 mb-2">{line.slice(4)}</h3>;
                if (line.trim().startsWith('- ')) {
                    const text = line.trim().slice(2);
                    return <div key={idx} className="flex items-start gap-2.5 my-1.5 pl-1"><span className="text-[#3bb5ff] mt-1.5 text-[8px]">•</span><span className="text-gray-300 text-xs flex-1">{parseInlineMarkdown(text)}</span></div>;
                }
...
function RemoteFolderPicker({ initialPath, onSelect, onCancel, showFiles = false, multiSelect = false, singleSelectOnly = false }) {
    ...
    const handleItemTap = (item) => {
        ...
        if (item.is_dir) {
            if (!multiSelect) fetchDirectory(item.path);
            else fetchDirectory(item.path);
        } else {
            if (multiSelect) toggleSelect(item.path);
...
export default function App() {
    ...
    const [externalVolumes, setExternalVolumes] = useState(() => { try { const saved = localStorage.getItem('mob_externalVolumes'); return saved ? JSON.parse(saved) : []; } catch { return []; } });
    const [recentVolumes, setRecentVolumes] = useState(() => { try { const saved = localStorage.getItem('mob_recentVolumes'); return saved ? JSON.parse(saved) : []; } catch { return []; } });
    const [toast, setToast] = useState(null);
    const [taskPasswordErrors, setTaskPasswordErrors] = useState({}); // taskId -> error message for wrong passwords
    const [lastPasswordErrorTaskId, setLastPasswordErrorTaskId] = useState(null); // tracks which task just had a bad password
    const [renameCollisionTasks, setRenameCollisionTasks] = useState(new Set()); // tasks that failed due to name collision
    ...
    const doImport = async (path, password) => {
        ...
        try {
            ...
            const isPwdErr = error.message && (error.message.toLowerCase().includes('password') || error.message.toLowerCase().includes('incorrect') || error.message.toLowerCase().includes('mac'));
            if (isPwdErr) {
                if (password !== null) {
                    setPasswordModal(prev => ({ ...prev, error: 'Incorrect password. Try again.', password: '' }));
                } else {
                    setPasswordModal({ open: true, path: path, password: '', error: '' });
                }
            } else {
                setPasswordModal({ open: false, path: '', password: '', error: '' });
    ...
    const executeShare = async (password = null) => {
        ...
        try {
            ...
            if (password || sharePassword) body.password = password || sharePassword;
    ...
    const connectWS = () => {
        ...
        socket.onmessage = e => {
            ...
            if (msg.type === 'stdout' || msg.type === 'stderr') {
                ...
                // Detects when the CLI reports a bad password so we can show a retry UI
                const pwdFailMatch = line.match(/(?:incorrect|wrong|invalid|bad|failed).*password|password.*(?:incorrect|wrong|invalid|bad|failed)|MAC verification failed|decryption failed|password_seed_hash mismatch|bad decrypt/i);
                if (pwdFailMatch && tid) {
                    setTaskPasswordErrors(prev => ({ ...prev, [tid]: 'The password was incorrect. Please try again.' }));
                    setLastPasswordErrorTaskId(tid);
                    // If a prompt is currently open for this task, inject the error into it
                    setPromptQueue(prev => prev.map(p => p.taskId === tid ? { ...p, error: 'The password was incorrect. Please try again.' } : p));
                    showToast('Password incorrect. Check your password and retry.', 'error');
                }

                // ─── RENAME COLLISION DETECTION ───
                // Detects when a rename fails because the target name already exists
                const renameFailMatch = line.match(/already exists|name collision|duplicate name|filename.*in use|target.*exists|rename.*failed.*exists/i);
                if (renameFailMatch && tid) {
                    showToast('Rename failed: A file or folder with that name already exists.', 'error');
                    setRenameCollisionTasks(prev => new Set(prev).add(tid));
                    setQueue(q => q.map(i => i.id === tid ? { ...i, status: 'failed', error: 'Name already exists' } : i));
                }
                    ...
                    setModal({
                        ...
                        content: (
                            <div className="space-y-4">
                                ...
                                <div className="p-3 bg-[#060d1a] border border-amber-500/30 rounded-xl font-mono text-xs text-amber-300 break-all">{fallbackNickname}</div>
                            </div>
                        ),
                ...
                // If this task just had a password error, carry the error into the new prompt
                const hadPasswordError = lastPasswordErrorTaskId === tid;
                if (hadPasswordError) {
                    setPromptQueue(prev => [...prev, {
                        text: msg.prompt,
                        isPassword: msg.is_password,
                        taskId: tid,
                        error: 'Previous password was incorrect. Please try again.'
                    }]);
                    setLastPasswordErrorTaskId(null); // consume the flag
                } else {
                ...
                if (msg.code === 0) {
                    ...
                    // Check if this failure was due to a wrong password
                    if (taskPasswordErrors[tid]) {
                        showToast('Operation failed: Incorrect password. Please retry with the correct password.', 'error');
                        setTaskPasswordErrors(prev => { const next = { ...prev }; delete next[tid]; return next; });
                    } else if (renameCollisionTasks.has(tid)) {
                        showToast('Operation failed: Name already exists in this folder.', 'error');
                        setRenameCollisionTasks(prev => { const next = new Set(prev); next.delete(tid); return next; });
                    } else {
    ...
    const sendWS = (action, args, task_id) => { if (wsRef.current?.readyState === WebSocket.OPEN) wsRef.current.send(JSON.stringify({ action, args, task_id })); };
            ...
            if (!r.ok || data.error) {
                ...
                if (path === '.') {
                    ...
                    if (retryCountRef.current < maxRetries) {
                        ...
                        setIsRetrying(true);
                        fetchTimeoutRef.current = setTimeout(() => fetchFiles(path, version), retryInterval);
                        return;
                    }
                }
    ...
    const fetchTermsStatus = async (retryCount = 0) => {
        try {
            const r = await fetch('/api/terms_status');
            if (!r.ok) throw new Error(`HTTP ${r.status}`);
            const data = await r.json();
            if (data.terms_accepted === 0) setShowTerms(true);
        } catch (e) {
            console.error('Failed to fetch terms status', e);
            if (retryCount < 5) setTimeout(() => fetchTermsStatus(retryCount + 1), 2000);
        }
    };
    ...
    // Normalize selectedDb at the source, not in an effect
    const normalizeDbName = (db) => {
        if (!db) return '';
        const isAbsolute = db.startsWith('/') || db.includes(':\\');
        if (isAbsolute) return db.endsWith('.db') ? db : `${db}.db`;
        const base = db.split(/[\/\\]/).pop();
        return base.endsWith('.db') ? base : `${base}.db`;
    };

    const selectDb = (db) => {
        const normalized = normalizeDbName(db);
        setSelectedDb(normalized);
        localStorage.setItem('mob_selectedDb', normalized);
        updateRecentVolumes(prev => [normalized, ...prev.filter(d => d !== normalized)].slice(0, 10));
    ...
    const displayedItems = searchQuery
        ? items.filter(i => i.displayName.toLowerCase().includes(searchQuery.toLowerCase()))
        : items;

    const toggleSelect = (item) => {
        setSelectedItems(prev => {
            const exists = prev.find(i =>
                (i.itemid && item.itemid && i.itemid === item.itemid) ||
                (i.displayName === item.displayName)
            );
            if (exists) return prev.filter(i =>
                !((i.itemid && item.itemid && i.itemid === item.itemid) || (i.displayName === item.displayName))
            );
            return [...prev, item];
        });
    };
    ...
    const ModifyModalContent = ({ type, item, items, onConfirm }) => {
        const targetItems = items || (item ? [item] : []);
        const isSingle = targetItems.length === 1;
        const targetItem = isSingle ? targetItems[0] : null;

        const [destination, setDestination] = useState('.');
        const [newName, setNewName] = useState(targetItem ? targetItem.displayName : '');
        ...
        return (
            <div className="space-y-4">
                <div className="bg-[#060d1a] p-3 rounded-lg border border-[#1a3a5c]">
                    ...
                    <p className="text-sm text-white truncate">{isSingle ? targetItem.displayName : `${targetItems.length} items`}</p>
                </div>
                {type === 'move' ? (
                    <>
                        <div>
    ...
    // Recursive encryption detection (ported from Mobile)
    ...
    const executeDownload = (itemsToDownload, strictnessMode = 'NA') => {
        ...
        const plainItems = itemsToDownload.filter(i => !needsPassword(i));
        ...
        if (plainItems.length) showToast(`${plainItems.length} download(s) queued`, 'success');

        if (encryptedItems.length) {
            const pwdItems = encryptedItems.map(item => ({
                id: item.itemid,
            ...
            setModal({
                ...
                content: <PasswordPromptModalContent items={pwdItems} onConfirm={(passwords) => {
                    setModal(null);
                    ...
                    showToast(`${encryptedItems.length} encrypted download(s) queued`, 'success');
                }} />
            });
        } else {
            setModal(null);
    ...
    const VolumeOptionsMenu = ({ db }) => (
        <div className="space-y-2">
            <button onClick={() => { setModal(null); selectDb(db); setTab('explorer'); }} className="w-full flex items-center gap-3 px-4 py-3 bg-[#0f1f3a] rounded-xl text-sm text-gray-300 btn-touch">{Ico.folderOpen} Open</button>
        ...
        return (
            <div className="space-y-4">
                <div>
                    ...
                    <input type="text" value={name} onChange={e => { setName(e.target.value); setError(''); }} placeholder="e.g. myvault"
                        className="w-full bg-[#060d1a] border border-[#1a3a5c] rounded-xl px-3 py-3 text-sm text-gray-200 outline-none focus:border-[#3bb5ff] transition-colors" autoFocus />
                    {error && <p className="text-xs text-red-400 mt-1">{error}</p>}
                    <p className="text-[10px] text-gray-600 mt-1">.db extension added automatically if omitted</p>
                </div>
                <div className="flex gap-3 pt-2">
                    ...
                    <button onClick={handleCreate} disabled={creating || !name.trim()} className="flex-1 py-3 bg-gradient-to-r from-[#006fbe] to-[#3bb5ff] text-white rounded-xl font-bold btn-touch text-sm disabled:opacity-40">
        ...
        return (
            <div className="flex flex-col h-full bg-[#060d1a]">
                <div className="flex items-center gap-1 px-3 py-2 bg-[#0a1628] border-b border-[#1a3a5c] overflow-x-auto">
                    ...
                    {currentVersion && <button onClick={() => { setCurrentVersion(null); setSelectedItems([]); fetchFiles(currentPath); }} className="ml-2 px-3 py-1.5 text-[9px] bg-orange-500/20 border border-orange-500/50 rounded-full text-orange-400 font-bold btn-touch animate-pulse">V:{currentVersion} ✕</button>}
                </div>

                {/* Search bar */}
                <div className="flex items-center gap-2 px-3 py-2 bg-[#0a1628] border-b border-[#1a3a5c]">
                    <div className="flex-1 flex items-center gap-2 bg-[#060d1a] border border-[#1a3a5c] rounded-xl px-3 py-2">
                        <span className="text-gray-500">{Ico.search}</span>
                        <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search files..." className="flex-1 bg-transparent text-sm text-gray-200 outline-none" />
                        {searchQuery && <button onClick={() => setSearchQuery('')} className="text-gray-500">{Ico.close}</button>}
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 pb-32 custom-scrollbar">
                    {volumeError ? (
                        ...
                        <div className="flex flex-col items-center justify-center h-full text-gray-600 opacity-40 py-20">
                            ...
                            <p className="text-sm font-bold uppercase tracking-widest">{searchQuery ? 'No matches found' : 'Folder is empty'}</p>
                        </div>
                    ) : (
                        <div className="grid grid-cols-[repeat(auto-fill,minmax(100px,1fr))] gap-3">
                            {displayedItems.map((item, idx) => {
                                ...
                                return (
                                    <div key={idx} onClick={() => toggleSelect(item)} onDoubleClick={() => { if (isFolder) { navigateTo(item.db_name || item.name); } }} className={`flex flex-col items-center justify-center p-4 rounded-2xl border transition-all btn-touch aspect-square relative ${isSelected ? 'bg-[#3bb5ff]/20 border-[#3bb5ff] shadow-lg scale-[0.98]' : 'bg-[#0a1628]/60 border-[#1a3a5c] hover:border-[#3bb5ff]/40'}`}>
        ...
        <div className="flex flex-col h-full">
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-6">
                    ...
                    <section>
                        ...
                        <div className="space-y-2">{recentVolumes.map(db => (
                            <div key={db} className={`flex items-center justify-between px-4 py-3 rounded-xl border btn-touch ${selectedDb === db ? 'bg-[#3bb5ff]/15 border-[#3bb5ff]' : 'bg-[#0f1f3a]/40 border-[#1a3a5c]'}`}>
                                <div onClick={() => { if (selectedDb === db) fetchFiles(currentPath); else selectDb(db); setTab('explorer'); }} className="flex-1 flex items-center gap-3 min-w-0 mr-2 py-1">
                                ...
                                <button onClick={(e) => { e.stopPropagation(); updateRecentVolumes(prev => prev.filter(d => d !== db)); if (selectedDb === db) setSelectedDb(''); showToast('Removed', 'success'); }} className="p-3 -m-1.5 text-gray-600 hover:text-red-400 rounded-lg transition-colors flex-shrink-0">{Ico.close}</button>
                            </div>
                        ))}</div>
                    </section>
                )}
                <section>
                    ...
                    <div className="space-y-2">{allVolumes.map(db => (
                        <div key={db} className={`flex items-center justify-between px-4 py-3 rounded-xl border btn-touch ${selectedDb === db ? 'bg-[#3bb5ff]/15 border-[#3bb5ff]' : 'bg-[#0f1f3a]/40 border-[#1a3a5c]'}`}>
                            <div onClick={() => { if (selectedDb === db) fetchFiles(currentPath); else selectDb(db); setTab('explorer'); }} className="flex-1 flex items-center gap-3 min-w-0 mr-2 py-1">
                                <span className={selectedDb === db ? 'text-[#3bb5ff]' : 'text-gray-500'}>{Ico.cube}</span>
                                <span className="text-sm font-bold text-white truncate">{db.split(/[\/\\]/).pop().replace('.db', '')}</span>
                            </div>
                            <button onClick={(e) => { e.stopPropagation(); setModal({ title: 'Volume Options', content: <VolumeOptionsMenu db={db} /> }); }} className="p-3 -m-1.5 text-gray-500 hover:text-white btn-touch">{Ico.menu}</button>
                        </div>
                    ))}</div>
                </section>
                <button onClick={() => setModal({ title: 'Import .vov Package', content: <div className="space-y-3"><RemoteFolderPicker showFiles onSelect={p => { if (p.endsWith('.vov')) { doImport(p, null); setModal(null); } else showToast('Must select .vov', 'error'); }} onCancel={() => setModal(null)} /></div> })} className="w-full py-4 bg-[#0f1f3a] border border-[#1a3a5c] rounded-2xl text-xs text-gray-300 font-bold uppercase btn-touch flex items-center justify-center gap-2">{Ico.import} Import VOV Package</button>
            </div>
        </div>
    );
    ...
    const renderQueue = () => (
        <div className="flex flex-col h-full">
            <div className="px-4 py-3 bg-[#0a1628] border-b border-[#1a3a5c] flex items-center justify-between"><h2 className="text-lg font-bold text-white">Queue</h2><button onClick={() => setQueue(q => q.filter(i => i.status === 'queued' || i.status === 'running'))} className="text-xs text-gray-400 btn-touch">Clear Done</button></div>
            <div className="flex-1 overflow-y-auto p-3 space-y-2">{queue.length === 0 ? <div className="text-center py-20 text-gray-500 text-sm">No active tasks</div> : queue.slice().reverse().map(item => (
                <div key={item.id} className="flex items-center gap-3 px-3 py-2 bg-[#0a1628] border border-[#1a3a5c] rounded-xl">
                    <div className={`w-2 h-2 rounded-full flex-shrink-0 ${item.status === 'completed' ? 'bg-green-400' : item.status === 'failed' ? 'bg-red-400' : item.status === 'running' ? 'bg-[#3bb5ff] animate-pulse' : 'bg-gray-500'}`} />
                    <div className="flex-1 min-w-0">
                        <div className="text-xs text-gray-200 truncate">{item.name}</div>
                        <div className="text-[10px] text-gray-500 capitalize flex items-center gap-1.5">
                            {item.status}
                            {item.status === 'failed' && item.error === 'Name already exists' && (
                                <span className="text-red-400">• Name collision</span>
                            )}
                            {item.status === 'failed' && taskPasswordErrors[item.id] && (
                                <span className="text-red-400">• Wrong password</span>
                            )}
                        </div>
                    </div>
                    <div className="flex items-center gap-2 flex-shrink-0">
                        {item.progress > 0 && item.status !== 'failed' && <div className="text-xs text-[#3bb5ff]">{item.progress}%</div>}
                        {item.status === 'failed' && taskPasswordErrors[item.id] && (
                            <button
                                onClick={() => {
                                    showToast('Re-select the item and retry with the correct password.', 'info');
                                    setTaskPasswordErrors(prev => { const next = { ...prev }; delete next[item.id]; return next; });
                                }}
                                className="px-2 py-1 bg-[#3bb5ff]/10 border border-[#3bb5ff]/30 rounded-lg text-[10px] text-[#3bb5ff] font-bold btn-touch"
                            >
                                Dismiss
                            </button>
                        )}
        ...
        <div className="flex flex-col h-full safe-top bg-[#060d1a]">
            ...
            <nav className="flex items-center justify-around bg-[#0a1628] border-t border-[#1a3a5c] py-4 safe-bottom shadow-[0_-10px_30px_rgba(0,0,0,0.4)]">
                {[
                    ...
                    { id: 'queue', ico: Ico.clock, label: 'TASKS', badge: queue.filter(q => q.status === 'running' || q.status === 'queued').length },
                    { id: 'terminal', ico: Ico.terminal, label: 'TERMINAL' },
                    { id: 'settings', ico: Ico.settings, label: 'SYSTEM' }
                ].map(n => (
                    <button
                        key={n.id}
                        onClick={() => setTab(n.id)}
                        className={`relative flex flex-col items-center gap-1.5 transition-all duration-300 ${tab === n.id ? 'text-[#3bb5ff] scale-110' : 'text-gray-600'}`}
                    >
                        ...
                        {n.badge > 0 && <span className="absolute -top-1 -right-1 w-5 h-5 bg-[#3bb5ff] text-[9px] text-[#0a1628] rounded-full flex items-center justify-center font-black border-2 border-[#0a1628]">{n.badge}</span>}
                    </button>
                ))}
            </nav>
            ...
            {showTerms && (
                <div className="fixed inset-0 z-[150] flex flex-col bg-[#060d1a]/97 backdrop-blur-lg overflow-y-auto safe-top safe-bottom">
                    <div className="flex flex-col max-w-md mx-auto w-full min-h-screen px-6 py-10 space-y-5">
                        <div className="flex flex-col items-center gap-4 pt-4">
                            <div className="relative">
                                <div className="absolute inset-0 rounded-2xl bg-[#3bb5ff]/20 blur-xl scale-150" />
                                <div className="relative w-16 h-16 rounded-2xl bg-gradient-to-br from-[#0f2a4a] to-[#061528] flex items-center justify-center border border-[#3bb5ff]/40 shadow-[0_4px_24px_rgba(59,181,255,0.3)]">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="#3bb5ff" strokeWidth="1.75" className="w-8 h-8">
                                        <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                    </svg>
                                </div>
                            </div>
                            <div className="text-center space-y-1.5">
                                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#3bb5ff]/10 border border-[#3bb5ff]/25 mb-1">
                                    <span className="w-1.5 h-1.5 rounded-full bg-[#3bb5ff] animate-pulse" />
                                    <span className="text-[10px] text-[#3bb5ff] font-bold uppercase tracking-[0.18em]">Required</span>
                                </div>
                                <h2 className="text-2xl font-black text-white tracking-tight">Terms &amp; Policy</h2>
                                <p className="text-xs text-gray-400 font-medium">Please read and accept before continuing</p>
                            </div>
                        </div>
                        <div className="flex-1 w-full bg-[#080f20] border border-[#1a3a5c] rounded-2xl shadow-2xl overflow-y-auto max-h-[52vh] custom-scrollbar">
                            <div className="p-5">
                                <MiniMarkdown content={TERMS_AND_CONDITIONS_MARKDOWN} />
                            </div>
                        </div>
                        <button onClick={handleAcceptTerms} className="w-full py-4 bg-gradient-to-r from-[#005ea8] to-[#3bb5ff] text-white rounded-2xl font-black uppercase tracking-widest text-xs btn-touch shadow-[0_4px_20px_rgba(59,181,255,0.35)] border border-[#3bb5ff]/40 cursor-pointer flex items-center justify-center gap-2.5">
                            <svg viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4 flex-shrink-0"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /></svg>
                ...
                <div className="fixed inset-0 z-[150] flex flex-col bg-[#060d1a]/97 backdrop-blur-lg overflow-y-auto safe-top safe-bottom">
                    <div className="flex flex-col max-w-md mx-auto w-full min-h-screen px-6 py-10 space-y-5">
                        <div className="flex flex-col items-center gap-4 pt-4">
                            ...
                            <div className="text-center space-y-1.5">
                                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#3bb5ff]/10 border border-[#3bb5ff]/25 mb-1">
                                    <span className="w-1.5 h-1.5 rounded-full bg-[#3bb5ff]" />
                                    <span className="text-[10px] text-[#3bb5ff] font-bold uppercase tracking-[0.18em]">Getting Started</span>
                                </div>
                                <h2 className="text-2xl font-black text-white tracking-tight">Setup Tutorial</h2>
                                <p className="text-xs text-gray-400 font-medium">Follow this quick guide to start</p>
                            </div>
                        </div>
                        <div className="w-full bg-[#080f20] border border-[#1a3a5c] rounded-2xl shadow-2xl p-5 flex flex-col gap-4">
                            <p className="text-sm text-gray-200 leading-relaxed font-semibold">New here? Watch this video:</p>
                            <div className="relative w-full aspect-video rounded-xl overflow-hidden border border-[#1a3a5c]/80 shadow-lg bg-black">
                                <iframe className="absolute inset-0 w-full h-full" src={WELCOME_YOUTUBE_VIDEO_URL} title="YouTube video player" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen></iframe>
                            </div>
                            <p className="text-xs text-gray-400 leading-relaxed font-medium">
                                Then set up your requirements: put the token and the channel id in the settings. Go to the volumes tab and hit the plus button to make a new volume and you will be all set.
                            </p>
                        </div>
                        <button onClick={() => { setShowWelcomeVideo(false); setTab('settings'); }} className="w-full py-4 bg-gradient-to-r from-[#005ea8] to-[#3bb5ff] text-white rounded-2xl font-black uppercase tracking-widest text-xs btn-touch shadow-[0_4px_20px_rgba(59,181,255,0.35)] border border-[#3bb5ff]/40 cursor-pointer flex items-center justify-center gap-2.5">
                            <svg viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4 flex-shrink-0"><path fillRule="evenodd" d="M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z" clipRule="evenodd" /></svg>
                            I Understand — Let's Set Up
                        </button>
                    </div>
                </div>
            )}
            {/* Prompt Modal — handles CLI interactive input including password retries */}
            {promptQueue.length > 0 && (
                <Modal
                    open
                    onClose={() => {
                        if (wsRef.current?.readyState === WebSocket.OPEN) {
                            wsRef.current.send(JSON.stringify({ action: 'input', data: 'cancel', task_id: promptQueue[0].taskId }));
                        }
                        setPromptQueue(prev => prev.slice(1));
                    }}
                    title={promptQueue[0].error ? '⚠️ Authentication Failed' : "Input Required"}
                >
                    <div className="space-y-4">
                        {promptQueue[0].error && (
                            <div className="p-3 bg-red-900/20 border border-red-500/30 rounded-xl flex items-start gap-3 animate-in fade-in">
                                <span className="text-red-400 mt-0.5">{Ico.alert}</span>
                                <div>
                                    <p className="text-sm text-red-400 font-bold">{promptQueue[0].error}</p>
                                    <p className="text-[10px] text-red-400/70 mt-1">If you cancel, the operation will fail.</p>
                                </div>
                            </div>
                        )}
                        <p className="text-sm text-gray-300">{promptQueue[0].text}</p>
                        <input
                            data-prompt-input
                            type={promptQueue[0].isPassword ? 'password' : 'text'}
                            autoFocus
                            ...
                            placeholder={promptQueue[0].isPassword ? "Enter password..." : "Enter response..."}
                            onKeyDown={e => {
                                if (e.key === 'Enter') {
                                    const val = e.target.value;
                                    if (wsRef.current?.readyState === WebSocket.OPEN) {
                                        wsRef.current.send(JSON.stringify({ action: 'input', data: val, task_id: promptQueue[0].taskId }));
                                    }
                                    setPromptQueue(prev => prev.slice(1));
                                }
                            }}
                        ...
                        <div className="flex gap-3">
                            <button
                                ...
                                className="flex-1 py-3 bg-[#0f1f3a] text-gray-300 rounded-xl border border-[#1a3a5c] font-bold btn-touch"
                            >
                                Cancel
                            </button>
                            <button
                                onClick={() => {
                                    const inp = document.querySelector('[data-prompt-input]');
                                ...
                                className="flex-1 py-3 bg-gradient-to-r from-[#006fbe] to-[#3bb5ff] text-white rounded-xl font-bold btn-touch"
                            >
                                Submit
                            </button>
                        </div>
